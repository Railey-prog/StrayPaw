...
export function MyReports() {
  ...
  const myReports = user
    ? reports.filter((r) => r.reporter_user_id === user.id)
    : [];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
      <div className="mb-8">
        ...
        <p className="text-slate-600 text-lg max-w-2xl">
          Track and manage the stray animals you've reported.
        </p>
      </div>
      ...
      {myReports.length > 0 ? (
        <motion.div
          ...
          {myReports.map((report) => (
            <motion.div key={report.id} variants={itemVariants} className="h-full flex flex-col">
              <div className="flex-1">
                <ReportCard report={report} />
              </div>
              <button
                onClick={() => navigate(`/reports/${report.id}/edit`)}
                className="mt-2 w-full flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-[#2D6A4F] hover:text-[#2D6A4F] py-2.5 rounded-xl font-semibold text-sm transition-all">
                <Pencil size={14} />
                Edit Report
              </button>
            </motion.div>
